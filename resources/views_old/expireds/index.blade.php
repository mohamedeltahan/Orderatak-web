...
<!--End side menu component-->
    ...
    <!---end of navbar !-->
        ...
        <table id="example" class="ui celled table table-fixed" style="text-align: center" dir="rtl"   >
            ...
            <th style="background: lightseagreen; color: white;">كمية</th>
            <th style="background: lightseagreen; color: white;">سعر الشراء</th>
            <th style="background: lightseagreen; color: white;"> جمالي المبلغ</th>
            ...
            @foreach($expireds as $expired)
                ...
                <tr  id="{{$expired->id}}" data-nameid="{{$expired->name->id}}" data-route="{{route("items.update",$expired->id)}}" style="font-weight: 500">
                <td class="name">{{$expired->name->name}}</td>
                <td class="code">{{$expired->name->code}}</td>
                <td class="color">{{$expired->name->color}}</td>
                <td class="size">{{$expired->size}}</td>
                <td class="quantity">{{$expired->quantity}}</td>
                <td class="buying_price">{{$expired->buying_price}}</td>
                <td class="" >{{$expired->quantity*$expired->buying_price}}</td>
