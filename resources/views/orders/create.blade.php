...
<body>
    ...
    <input value="{{route("get_all_phone_numbers")}}" id="get_all_phone_numbers_link" class="d-none">
    <input value="{{route("get_phone_details")}}" id="get_phone_details_link" class="d-none">
                ...
                <div class="clientinfocontainer">
                    <div class="clientinfo">
                        ...
                        <form autocomplete="off" id="receiptclientinfoform" >
                            ...
                            <input data-alertmessage="من فضلك ادخل اسم العميل" class="defv all" type="text" placeholder="اسم العمبل" id="clientname" name="clientname" autocomplete="off">
                            <input list="phone_numbers"  data-alertmessage="من فضلك ادخل رقم هاتف صحيح" type="text" placeholder="رقم العميل" data-length="11" clasefv all paste" id="clientnumber">
                            <datalist id="phone_numbers">
                            
                              </datalist>
                            
                            <input list="addresses_list"  data-alertmessage="من فضلك ادخل عنوان العميل" type="text" placeholder="عنوان العميل" id="client_address"  class="deff all" autocomplete="off"><br>
...
<script>
    $(document).ready(function() {
        ...
        $(".search").keyup(function (e) {
                if(e.key==="Backspace"){
                    ...
                    $(".warning").addClass("no-result");
                    return;
                }


                var temp=$(this).parent().parent();
                temp.find(".iteem").addClass("no-result");
                var value=$(this).val();
                var is_found=false;
                temp.find(".iteem").each(function () {
                   if(($(this).find(".name").text().indexOf(value)!=-1) || ($(this).find(".size").text().indexOf(value)!=-1) || ($(this).find(".code").text().indexOf(value)!=-1)){
                       is_found=true;
                       $(this).removeClass("no-result");
                   }
               });
               if(is_found===false){
                  $(".warning").removeClass("no-result");
    ...
    $(document).on("keyup","#clientnumber",function () {
        var val=$(this).val();
        if(val.length>5){
         $.get($("#get_all_phone_numbers_link").val(),{"phone":val},function(data, status){
         var arr=data;
         $('#phone_numbers').html("");
         for(var i=0 ; i<data.length;i++){
             $('#phone_numbers').append($('<option>', {
                value: data[i],
                text: data[i]
            }));
         }
    ...
    $(document).on("focusout","#clientnumber",function () {
        var val=$(this).val();
        if(val.length>5){
         $.get($("#get_phone_details_link").val(),{"phone":val},function(data, status){
         var arr=data;
         var addresses=data["addresses"];
         $("#addresses_list").html("");
         for(var i=0 ; i<addresses.length;i++){
             $('#addresses_list').append($('<option>', {
         ...
         $("#customer_link").val(data["customer_link"]);
    ...
    $("#searchbox").on("focusout",function(){
        ...
        $(".productname").each(function(){
            if(val===$(this).text()){
                ...
                $(this).parent().click();
              //  alert( $(this).parent().parent().parent().parent().parent().parent().find(".btn-link").attr("class"));

              //  $(this).parent().parent().parent().parent().parent().parent().parent().find(".card-header .btn-link").addClass("colorred");
                           //     $(this).parent().parent().parent().parent().parent().parent().parent().find(".card-header .btn-link").click();
