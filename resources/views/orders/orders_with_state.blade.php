...
<body>
  ...
  @if(isset($ship_id))
     <input class="url" value="{{route("get_for_ajax_with_state",$state)}}?ship_id={{$ship_id}}" style="display: none">
  @else
     <input class="url" value="{{route("get_for_ajax_with_state",$state)}}" style="display: none">
  ...
  <input id="order_details" value="{{route("order_details","")}}" style="display: none">
   ...
   <a href="" style="display: none" id="a">
    ...
    <button id="span_to_click"></button></a>
 ...
 <!--End side menu component-->
    ...
    <div style="width: 82%" id="main_div">
        ...
        @section('content')
            ...
            <li class="breadcrumb-item" ><a href="{{route("orders_with_state",$state)}}"> اوردرات {{$state}}</a></li>
       ...
       <!---end of navbar !-->
        ...
        <div class="exporters-main-container col-12">
                <div class="exports_cases" style="width: 80%; margin-top: 55px; display: inline-flex; margin-right: 23%" >
                    <div><button id='filter_tag' class="headers modal_tag" data-table="users" style="border-top-right-radius: 22px;border-bottom-right-radius: 22px; height: 50px;width: 225px; color: steelblue;">فلترة النتائج<i style="margin-right: 6px;" class="fas fa-filter fa-2x"></i></button>
                        ...
                        <button id='excel_tag' class="headers modal_tag" data-table="items" style="border-top-left-radius: 22px;border-bottom-left-radius: 22px;height: 50px;width: 225px; color: forestgreen;">تصدير لاكسيل<i style="margin-right: 6px;" class="fas fa-file-csv fa-2x"></i></button>
                    </div>
                </div>
                <div class="exporters-container col-12" style="margin-top: 5%" >
                    <div class="search-container search_tag tags none"  style=" width: 100%; margin-bottom: 45px !important; ">
                        ...
                        <form  id="searchbox" style="width: 68%; margin-right: 23%" dir="rtl">
                    ...
                    <div class="inp-cont filter_tag tags none" style="width: 92%; margin-right: 78px; margin-left: -10%;"  >
                        <div class="input-group mb-2">
                            ...
                            <select name="type" id="select"  style="width: 23%;height: 54px;">
                                ...
                                <option @if($state=="الكل")selected @endif value="{{route("ship_orders",$ship_id)}}" >الكل</option>
                                <option @if($state=="انتظار")selected @endif value="{{route("orders_with_state","انتظار")}}?ship_id={{$ship_id}}">انتظار</option>
                                <option @if($state=="لم يتم التاكيد")selected @endif value="{{route("orders_with_state","لم يتم التاكيد")}}?ship_id={{$ship_id}}">لم يتم التاكيد</option>
                                <option @if($state=="تم التاكيد")selected @endif value="{{route("orders_with_state","تم التاكيد")}}?ship_id={{$ship_id}}"> تم التاكيد</option>
                                <option @if($state=="تم التسليم لشركة الشحن")selected @endif value="{{route("orders_with_state","تم التسليم لشركة الشحن")}}?ship_id={{$ship_id}}">تم التسليم لشركة الشحن</option>

                                <option @if($state=="تم الشحن")selected @endif value="{{route("orders_with_state","تم الشحن")}}?ship_id={{$ship_id}}">تم الشحن </option>
                                <option @if($state=="مرتجع جزئي")selected @endif value="{{route("orders_with_state","مرتجع جزئي")}}?ship_id={{$ship_id}}"> مرتجع جزئي</option>

                                <option @if($state=="مرتجع")selected @endif value="{{route("orders_with_state","مرتجع")}}?ship_id={{$ship_id}}"> مرتجع</option>
                            @else

                                <option @if($state=="الكل")selected @endif value="{{route("orders.index")}}" >الكل</option>
                                <option @if($state=="انتظار")selected @endif value="{{route("orders_with_state","انتظار")}}">انتظار</option>
                                <option @if($state=="لم يتم التاكيد")selected @endif value="{{route("orders_with_state","لم يتم التاكيد")}}">لم يتم التاكيد</option>


                                <option  @if($state=="تم التاكيد")selected @endif value="{{route("orders_with_state","تم التاكيد")}}"> تم التاكيد</option>
                                <option @if($state=="تم التسليم لشركة الشحن")selected @endif  value="{{route("orders_with_state","تم التسليم لشركة الشحن")}}">تم التسليم لشركة الشحن</option>

                                <option @if($state=="تم الشحن")selected @endif  value="{{route("orders_with_state","تم الشحن")}}">تم الشحن </option>
                                <option @if($state=="مرتجع جزئي")selected @endif  value="{{route("orders_with_state","مرتجع جزئي")}}"> مرتجع جزئي</option>

                                <option @if($state=="مرتجع")selected @endif  value="{{route("orders_with_state","مرتجع")}}"> مرتجع</option>
                            @endif
                                ...
                                <div class="months">
                                    @foreach($days as $day=>$items)
                                        ...
                                        <div class="month co-2">
                                            <div class="ordersdate" data-attr="{{route("get_for_ajax_with_state",$day)}}"><span class="{{$day}}">{{$day}}</span></div>
                                            ...
                                            <div class="monthfooter">
                                                    <span dir="rtl">
                                                        <span>اجمالي اليوم : </span><span class="num"></span> <span class="sign">     {{{\Illuminate\Support\Facades\Auth::user()->currency}}}   </span><span> 
                                                        <br>
                                                    
                                                        </span><span> {{$items_count[$day]}} </span><span>
                                                            @if(isset($ship_id))
...
<table  style="display: none">
    <tr id="temp" class="orderdetails noborderright">
        ...
        <td class="co-1 number hovered "><a class="link" href="#"></a></td>
...
<script>
    ...
    $("#example .hovered").mouseover(function () {
        alert("dd");
        $(this).parent().find("hovered").css("background-color","red");
    });
