...
<!--End side menu component-->
    ...
    <!---end of navbar !-->
                ...
                <option data-link="{{route('restoreds.index')}}" @if($flag_that_all_items_are_passed==="true") selected @endif>كل المخازن</option>

                @foreach(App\Store::all() as $store)
                 <option data-link="{{route('restoreds.index')}}?store_id={{$store->id}}" @if($flag_that_all_items_are_passed==$store->id) selected @endif value="{{$store->id}}"> {{$store->name}} </option>
                @endforeach
              </select>
        ...
        <table id="example" class="ui celled table" style="text-align: center" dir="ltr"   >
            <thead >
            <tr >
                <th style="background: #5C71A3; color: white;">تاكيد</th>
                <th style="background: #5C71A3; color: white;" >سبب الاسترجاع</th>
                <th style="background: #5C71A3; color: white;" >تاريخ الاسترجاع</th>
...
<script >
    ...
    $("#add_item_button").click(function () {
     ...
     $(document).on("change","#stores_list",function () {
        var val=$(this).find(":selected").attr("data-link");
